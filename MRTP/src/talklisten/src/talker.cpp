...
#include <example_interfaces/msg/string.hpp> // needed because we publish strings
...
int main(int argc,char **argv) {
  ...
  pub = nodeh->create_publisher<example_interfaces::msg::String>("message",1);

  int counter = 0;
  while ( (counter++ < 100) && (rclcpp::ok())  ) {
    RCLCPP_INFO(nodeh->get_logger(),"Sending message #%d",counter);
    example_interfaces::msg::String stringtosend;
